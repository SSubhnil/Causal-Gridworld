...
def Q_learn(env, num_episodes, alpha, epsilon, greedy_interval, gamma=1.0):
    ...
    for i_episode in range(1, num_episodes + 1):
        ...
        if greedy_flag == 0:
            ...
            # epsilon decay - quadrant
            # eps = math.sqrt(1 - math.pow(percent_completion,2))
        else:
            eps = greedy_eps
...
for i in range(0, len(alpha)):
    for j in range(0, len(num_episodes)):
        Q_sarsa, step_count, greedy_step_count, Q_store = Q_learn(env, num_episodes[j], alpha[i], starting_eps, greedy_interval[j])
        running_average = moving_average(step_count)
        
        np.save("Q-learn-King-Wind-GW-Q-val-new_{}.npy".format(experiment_number), Q_store)
        np.save("Q-learn-King-Wind-GW-Step-count-new_{}.npy".format(experiment_number), step_count)
        # print("Q_sarsa\n", Q_sarsa)
        ...
        plt.savefig('Q-learn-King-Wind-GW-new_{}.png'.format(experiment_number), dpi = 1000)
        experiment_number += 1
