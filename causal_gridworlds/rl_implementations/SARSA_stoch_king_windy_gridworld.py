# -*- coding: utf-8 -*-
"""
Created on Sat Dec  19 18:24:16 2022
...
alpha = [0.25, 0.4, 0.5]
num_episodes = [20000, 40000, 60000]
starting_eps = 1.0
greedy_interval = [2000, 4000, 6000]
experiment_number = 1

for i in range(0, len(alpha)):
    for j in range(0, len(num_episodes)):

        Q, step_count, greedy_step_count, avg_greedy_step_count, Q_store, greedy_wind_profile = sarsa(env, num_episodes[j], alpha[i], starting_eps, greedy_interval[j])
        running_average = moving_average(step_count)
        
        
        np.save("SARSA-Stoch-King-Windy-GW-Q_values-greedy_eval-{}.npy".format(experiment_number), Q_store)
        np.save("SARSA-Stoch-King-Windy-GW-Wind_Profile-greedy_eval-{}.npy".format(experiment_number), greedy_wind_profile)
        # print("Q_sarsa\n", Q_sarsa)
        ...
        ax2.plot(spacer2, avg_greedy_step_count, color=color, label ="Avg. Greedy Eval.")
        for x,y in zip(spacer2, avg_greedy_step_count):
            ax2.annotate('%s' % y, xy=(x,y), textcoords = 'data')
        ax2.tick_params(axis='y', labelcolor=color)
        plt.title('SARSA-Stoch-King-Windy-GW alp=%f' % alpha[i] )
        plt.legend(loc="upper right")
        plt.savefig('SARSA-Stoch-King-Windy-GW-greedy_eval-{}.png'.format(experiment_number), dpi = 1000)
        
        #%%
        plt.figure()
        plt.title("SARSA-Stoch-King Greedy Evaluations ")
        ...
        plt.legend()
        plt.savefig("SARSA-Stoch-King-Windy-GW-greedy_episodes-{}.png".format(experiment_number), dpi = 1000)
        experiment_number += 1
