# -*- coding: utf-8 -*-
"""
Created on Fri Dec  9 11:51:55 2022

@author: SSubhnil

@details: TD-SARSA for Stochastic-Windy-gridworld
"""
import numpy as np
...
alpha = [0.5, 0.4, 0.25, 0.1]
num_episodes = [10000, 20000, 30000]
starting_eps = 1.0
greedy_interval = [1000, 2000, 3000]
experiment_number = 1

for i in range(0, len(alpha)):
    for j in range(0, len(num_episodes)):
        Q_sarsa, step_count, greedy_step_count, avg_greedy_step_count, Q_store, greedy_wind_profile = sarsa(env, num_episodes[j], alpha[i], starting_eps, greedy_interval[j])
        running_average = moving_average(step_count)
        
        np.save("SARSA-Stoch-Windy-GW-Q-val-greedy_eval-{}.npy".format(experiment_number), Q_store)
        np.save("SARSA-Stoch-Windy-GW-greedy_wind_profile-greedy_eval-{}.npy".format(experiment_number), greedy_wind_profile)
        np.save("SARSA-Stoch-Windy-GW-Step_count-Greedy_eval-{}.npy".format(experiment_number), step_count)
        np.save("SARSA-Stoch-Windy-GW-Greedy_Step_count-greedy_eval-{}.npy".format(experiment_number), greedy_step_count)
        # print("Q_sarsa\n", Q_sarsa)
        ...
        color = 'tab:blue'
        ax2.set_ylabel('Avg. Greedy Evals (steps/batch)', color=color)
        ...
        plt.title('SARSA-Stoch-Windy-GW-greedy_eval alp=%f' % alpha[i] )
        plt.savefig('SARSA-Stoch-Windy-GW-greedy_eval-{}.png'.format(experiment_number), dpi=1000)
        ...
        plt.legend()
        plt.savefig("SARSA-Stoch-Windy-GW-greedy_episodes.png", dpi = 1000)
