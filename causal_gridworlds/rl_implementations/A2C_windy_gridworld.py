...
run = wandb.init(project="A2C-4A-Windy-GW")
...
class A2C:
    ...
    def train(self, state, action, reward, next_state, done):
        ...
        if len(self.replay_buffer.buffer) == self.buffer_size-1:
...
if __name__ == "__main__":
    ...
    wandb.watch(agent.actor, log='gradients', log_freq = 1000, idx = 1, log_graph = True)
    wandb.watch(agent.critic, log='gradients', log_freq = 1000, idx = 2, log_graph = True)
    ...
    for episode in range(num_episodes):
        ...
        try:
            wandb.log({'Reward':episode_reward,\
                  'Learning rate':learning_rate_actor})
        except:
    ...
    plt.title('A2C-4A-GW alp=%f' % learning_rate_actor)
    plt.legend(loc="upper right")
    plt.savefig('A2C-4A-GW-test_h{}_{}.png'.format(hidden_dim, experiment_number), dpi=600)
